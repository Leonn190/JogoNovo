...
class WorldPhysics:
    def __init__(
        ...
        self.collision_damage_factor = float(collision_damage_factor)
        self.dead_damping = float(dead_damping)
    ...
    def _integrate(self, b: Ball, dt: float):
        if not b.alive():
            b.vx *= self.dead_damping
            b.vy *= self.dead_damping
    ...
    def _resolve_ball_ball(self, a: Ball, b: Ball):
        ...
        self._positional_correction(a, b, nx, ny, penetration)
        ...
        a_to_b = a.vx * nx + a.vy * ny
        b_to_a = -(b.vx * nx + b.vy * ny)
    ...
    def _positional_correction(self, a: Ball, b: Ball, nx: float, ny: float, penetration: float):
        inv_mass_sum = a.inv_mass + b.inv_mass
        if inv_mass_sum <= 0:
            return

        correction = max(penetration - self.penetration_slop, 0.0)
        correction = min(correction, self.max_correction)
        corr = correction / inv_mass_sum
    ...
    def _apply_damage(self, impulse_mag: float, hitter: Ball, target: Ball):
        ...
        base_damage = hitter.compute_contact_damage()
        impulse_bonus = impulse_mag * self.collision_damage_factor
        damage = base_damage + impulse_bonus
